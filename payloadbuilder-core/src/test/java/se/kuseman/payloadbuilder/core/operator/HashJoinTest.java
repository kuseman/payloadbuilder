...
public class HashJoinTest extends AOperatorTest
{
    ...
    {
        HashJoin op = new HashJoin(0, "", op(c -> emptyIterator()), op(c -> emptyIterator()), (c, tuple) -> 0, (c, tuple) -> 0, ctx -> false, new DefaultTupleMerger(-1, 1, 2), false, false);
    ...
    public void test_inner_join_no_populate()
    {
        ...
        String query = "select * " + "from tableA a " + "inner join range(1, 6) b " + "  on b.Value = a.col1";
        ...
        Map<String, Function<TableAlias, Operator>> opByTable = MapUtils.ofEntries(MapUtils.entry("tableA", a -> op(ctx -> IntStream.of(1, 2, 3, 4, 5)
                ...
                .iterator(), () -> aCloseCount.increment())), MapUtils.entry("tableB", a -> op(
                        ctx -> IntStream.of(4, 5, 6, 7)
                                .mapToObj(i -> (Tuple) Row.of(a, new String[] { "pos", "col1", "col2" }, new Object[] { 10 * i, i }))
                                .iterator(),
    ...
    public void test_inner_join_populate()
    {
        ...
        String query = "select * " + "from range(1, 10) a " + "inner join tableB b with (populate=true) " + "  on b.Value = a.Value";

        Map<String, Function<TableAlias, Operator>> opByTable = MapUtils.ofEntries(MapUtils.entry("tableB", a -> op(context -> IntStream.range(1, 20)
                .mapToObj(i -> (Tuple) Row.of(a, new String[] { "pos", "Value" }, new Object[] { i - 1, i % 10 }))
                .iterator(), () -> leftClose.increment())));
        ...
        while (it.hasNext())
        {
            ...
            assertArrayEquals(new int[] { count, count + 10 }, stream(col).mapToInt(t -> (int) t.getValue(0))
                    .toArray());
            count++;
        }
        it.close();
    ...
    public void test_outer_join_no_populate()
    {
        ...
        String query = "select * " + "from tableA a " + "left join tableB b " + "  on b.Value = a.Value";

        Map<String, Function<TableAlias, Operator>> opByTable = MapUtils.ofEntries(MapUtils.entry("tableA", a -> op(context -> IntStream.range(0, 10)
                .mapToObj(i -> (Tuple) Row.of(a, new String[] { "pos", "Value" }, new Object[] { i, i }))
                .iterator(), () -> leftClose.increment())), MapUtils.entry("tableB", a -> op(
                        context -> IntStream.range(5, 15)
                                .mapToObj(i -> (Tuple) Row.of(a, new String[] { "pos", "Value" }, new Object[] { i, i }))
                                .iterator(),
    ...
    public void test_outer_join_populate()
    {
        ...
        String query = "select * " + "from tableA a " + "left join tableB b with (populate = true) " + "  on b.Value = a.Value";

        Map<String, Function<TableAlias, Operator>> opByTable = MapUtils.ofEntries(MapUtils.entry("tableA", a -> op(context -> IntStream.range(0, 10)
                .mapToObj(i -> (Tuple) Row.of(a, new String[] { "pos", "Value" }, new Object[] { i, i }))
                .iterator(), () -> leftClose.increment())), MapUtils.entry("tableB", a -> op(
                        context -> IntStream.range(5, 15)
                                .mapToObj(i -> (Tuple) Row.of(a, new String[] { "pos", "Value" }, new Object[] { i - 1, i % 5 + 2 }))
                                .iterator(),
                        () -> rightClose.increment())));
        ...
        int[][] tableBPos = new int[][] { new int[] { 4, 9 }, new int[] { 5, 10 }, new int[] { 6, 11 }, new int[] { 7, 12 }, new int[] { 8, 13 } };
        ...
        while (it.hasNext())
        {
            ...
            if (pos >= 2
                    && pos <= 6)
            {
                assertArrayEquals("Count: " + count, tableBPos[pos - 2], stream(col).mapToInt(t -> (int) t.getValue(0))
                        .toArray());
            }
            else
            {
