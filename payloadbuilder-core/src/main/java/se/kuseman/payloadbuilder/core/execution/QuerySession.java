...
public class QuerySession implements IQuerySession
{
    ...
    private Map<String, TemporaryTable> temporaryTables;
    private TempTableCache tempTableCache = new InMemoryTempTableCache();
    private GenericCache genericCache = new InMemoryGenericCache();
    ...
    public QuerySession(CatalogRegistry catalogRegistry, Map<String, Object> variables)
    {
        ...
        this.variables = requireNonNull(variables, "variables").entrySet()
                .stream()
                .collect(toMap(k -> k.getKey(), v -> ValueVector.literalAny(v.getValue())));
    }
    ...
    public TemporaryTable getTemporaryTable(String table)
    {
     ...
     */
    public void setTemporaryTable(String name, TemporaryTable table)
    {
    ...
    public Collection<Entry<String, TemporaryTable>> getTemporaryTables()
    {
        if (temporaryTables == null)
        {
            return emptyList();
    ...
    public ValueVector getSystemProperty(String name)
    {
        ...
        ValueVector property = systemProperties.get(lowerCase(name));
        if (property == null)
        {
            return ValueVector.literalNull(ResolvedType.of(Column.Type.Any), 1);
        }
