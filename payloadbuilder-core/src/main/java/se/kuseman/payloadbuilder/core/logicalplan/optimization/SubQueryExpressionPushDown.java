...
public class SubQueryExpressionPushDown extends ALogicalPlanOptimizer<SubQueryExpressionPushDown.Ctx>
{
    ...
    protected ILogicalPlan create(ConstantScan plan, Ctx context)
    {
        ...
        for (List<IExpression> columns : plan.getRowsExpressions())
        {
            ...
            {
                logicalPlans.add(new ConstantScan(plan.getSchema(), List.of(columnsResult), plan.getLocation()));
            }
        }
        ...
        if (allConstantScans)
        {
            return new ConstantScan(plan.getSchema(), logicalPlans.stream()
        ...
        return new Concatenation(logicalPlans, plan.getLocation());
    }
            ...
            if (plan instanceof ConstantScan cs)
            {
                        ...
                        .size() == 1)
                {
                    return false;
                        ...
                        .size() > 1)
                {
                    throw new ParseException("A sub expression must return a single row", expression.getLocation());
                }
            }
