...
import se.kuseman.payloadbuilder.core.operator.TemporaryTable.Row;
...
public class TemporaryTableScanOperator extends AOperator
{
    ...
    public TupleIterator open(IExecutionContext ctx)
    {
        ...
        TemporaryTable temporaryTable = context.getSession()
                .getTemporaryTable(table.getTable());
        if (table == null)
        {
            throw new OperatorException("Data for temporary table " + table.getTable() + " could not be found");
        }
        final Iterator<TemporaryTable.Row> it = temporaryTable.getRows()
                .iterator();
        return new TupleIterator()
        {
            ...
            {
                Row row = it.next();
                return new TemporaryTableTuple(table.getTableAlias()
                        .getTupleOrdinal(), temporaryTable.getColumns(), row.getValues());
            }
    ...
    public boolean equals(Object obj)
    {
        if (obj instanceof TemporaryTableScanOperator)
        {
            ...
            return table.getTable()
                    .equals(that.table.getTable())
                    && table.isTempTable() == that.table.isTempTable()
                    && table.getTableAlias()
                            .getTupleOrdinal() == that.table.getTableAlias()
                                    .getTupleOrdinal()
