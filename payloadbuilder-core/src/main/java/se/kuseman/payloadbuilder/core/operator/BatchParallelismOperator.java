...
/**
 * Operator used along with Batch-operators when Parallelism is wanted NOTE! Experimental status
 * 
 * <pre>
...
class BatchParallelismOperator extends AOperator
{
    ...
    BatchParallelismOperator(int nodeId, Operator outer, Operator inner, int targetOuterNodeId, int batchSize)
    {
        super(nodeId);
    ...
    private ExecutionContext startBatch(ArrayBlockingQueue<Tuple> queue, IExecutionContext parent, List<Tuple> outerBatch, AtomicBoolean abort, AtomicInteger latch)
    {
        ExecutionContext childContext = ((ExecutionContext) parent).copy();
        BatchParallelismNodeData nodeData = childContext.getStatementContext()
                .getOrCreateNodeData(targetOuterNodeId, BatchParallelismNodeData::new);
    ...
    @Override
    public TupleIterator open(final IExecutionContext ctx)
    {
            ...
            public void close()
            {
                ...
                for (ExecutionContext childContext : childContexts)
                {
                    context.getStatementContext()
                            .mergeNodeData(childContext.getStatementContext());
                }
            }
        };
    }
    ...
    static class BatchhParallelismOuterOperator extends AOperator
    {
        ...
        {
            final BatchParallelismNodeData data = context.getStatementContext()
                    .getNodeData(nodeId);
            return new TupleIterator()
            {
                int index;
