...
abstract class ASessionCacheProvider<TValue> implements CacheProvider
{
    ...
    public List<CacheInfo> getCaches()
    {
        return caches.entrySet()
                .stream()
                .map(e -> new CacheInfo(e.getKey(), e.getValue().elements.size(), e.getValue().cacheHits, e.getValue().cacheMisses))
                .collect(toList());
    }
    ...
    public List<CacheEntryInfo> getCacheEntries(QualifiedName cacheName)
    {
        ...
        return cache.elements.entrySet()
                .stream()
                .map(e -> new CacheEntryInfo(e.getKey(), e.getValue().expireTime >= 0 ? ZonedDateTime.ofInstant(Instant.ofEpochMilli(e.getValue().expireTime), ZoneId.systemDefault())
                        : null))
                .collect(toList());
    }
