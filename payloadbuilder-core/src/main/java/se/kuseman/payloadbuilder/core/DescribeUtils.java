...
public class DescribeUtils
{
    ...
    static Pair<Operator, Projection> getDescribeSelect(ExecutionContext context, Operator operator, Projection projection)
    {
        ...
        TableAlias alias = TableAliasBuilder.of(-1, TableAlias.Type.TABLE, QualifiedName.of("describe"), "d")
                .build();
    ...
    {
        return new RootProjection(columns, IntStream.range(0, columns.size())
                .mapToObj(index -> (Projection) (writer, ctx) ->
                {
                    Tuple tuple = ctx.getStatementContext()
                            .getTuple();
    ...
    private static void collectDescribableRows(ExecutionContext context, List<DescribableRow> rows, DescribableNode parent, int pos, String indent, boolean last)
    {
