...
public class PredicateAnalyzer
{
    ...
    public static AnalyzeResult analyze(Expression predicate, TableAlias tableAlias)
    {
        ...
        queue.add(predicate);
        List<IAnalyzePair> resultItems = new ArrayList<>();
    ...
    public static class AnalyzeResult
    {
        ...
        private final List<IAnalyzePair> pairs;

        public AnalyzeResult(List<IAnalyzePair> pairs)
        {
            this.pairs = requireNonNull(pairs);
        }

        public List<IAnalyzePair> getPairs()
        {
            return pairs;
        }
         ...
         * </pre>
         */
        Pair<List<IAnalyzePair>, AnalyzeResult> extractPushdownPairs(String alias)
        {
         ...
         * </pre>
         */
        Pair<List<IAnalyzePair>, AnalyzeResult> extractPushdownPairs(String alias, boolean isNullAllowed)
        {
            if (pairs.isEmpty())
            {
            ...
            int size = pairs.size();
            List<IAnalyzePair> leftovers = new ArrayList<>(size);
            List<IAnalyzePair> pushdowns = new ArrayList<>(size);
            for (int i = 0; i < size; i++)
            {
                AnalyzePair pair = (AnalyzePair) pairs.get(i);
        ...
        public Expression getPredicate()
        {
            ...
            for (int i = size - 1; i >= 0; i--)
            {
                ...
                {
                    result = new LogicalBinaryExpression(LogicalBinaryExpression.Type.AND, result, pair.getPredicate());
                }
            }
            return result;
        }
    ...
    public static class AnalyzePair implements IAnalyzePair
    {
        ...
        AnalyzePair(IAnalyzePair.Type type, IComparisonExpression.Type comparisonType, AnalyzeItem left, AnalyzeItem right)
        {
        ...
        @Override
        public IAnalyzePair.Type getType()
        {
            return type;
        }
         ...
         */
        @Override
        public QualifiedName getQualifiedName(String alias)
        {
            if (left.qname != null
        ...
        @Override
        public IExpression getComparisonExpression(String alias)
        {
            if (type != Type.COMPARISION)
            {
                throw new IllegalArgumentException("Pair is not of type COMPARISION");
            }
            return getExpressionPair(alias).getRight();
        }

        @Override
        public IInExpression getInExpression(String alias)
        {
            if (type != Type.IN)
            {
                throw new IllegalArgumentException("Pair is not of type IN");
            }
            return (IInExpression) getExpressionPair(alias).getRight();
        }

        @Override
        public ILikeExpression getLikeExpression(String alias)
        {
            if (type != Type.LIKE)
            {
                throw new IllegalArgumentException("Pair is not of type LIKE");
            }
            return (ILikeExpression) getExpressionPair(alias).getRight();
        }

        @SuppressWarnings("unchecked")
        @Override
        public <T extends IExpression> T getUndefinedValueExpression(Class<T> clazz)
        {
            if (type != Type.UNDEFINED)
            {
                throw new IllegalArgumentException("Pair is not of type UNDEFINED");
            }
            IExpression expression = left.expression;
        ...
        static AnalyzePair of(TableAlias tableAlias, Expression expression)
        {
            if (expression instanceof ComparisonExpression)
            {
                ...
                boolean sameAlias = !leftItem.aliases.isEmpty()
                        && Objects.equals(leftItem.aliases, rightItem.aliases);
            ...
            else if (expression instanceof NullPredicateExpression)
            {
                ...
                if (npe.getExpression() instanceof QualifiedReferenceExpression)
                {
                    ...
                    return new AnalyzePair(npe.isNot() ? Type.NOT_NULL
                            : Type.NULL, leftItem, rightItem);
                }
            }
        ...
        private static AnalyzeItem getQualifiedItem(TableAlias tableAlias, Expression expression)
        {
            if (expression instanceof QualifiedReferenceExpression)
            {
                ...
                QualifiedName qname = qre.getQualifiedName();
                 ...
                 * Alias hierarchy a - b
                 *
                 * a.col <-- Ok alias reference a.b.col <-- Not ok reference since b is a child a.c.col <-- Ok since c is not a child alias
                 *
                 */
    ...
    public static class AnalyzeItem
    {
        ...
        AnalyzeItem(Expression expression, Set<String> aliases, QualifiedName qname)
        {
    ...
    private static class QualifiedReferenceVisitor extends AExpressionVisitor<Void, QualifiedReferenceVisitor.Context>
    {
         ...
         */
        static void getAliases(TableAlias tableAlias, Expression expression, Set<String> result)
        {
        ...
        {
            Pair<String, String> pair = getAlias(context.tableAlias, expression.getQualifiedName());
            if (pair.getKey() != null)
            {
