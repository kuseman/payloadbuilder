...
class OrderBySelectItemRewriter implements SelectRewriter<Select>
{
    ...
    {
        if (source.getOrderBy()
                .isEmpty())
        {
            return source;
        }

        int size = source.getSelectItems()
                .size();
        ...
        {
            SelectItem item = source.getSelectItems()
                    .get(i);

            if (firstAsteriskOrdinal == -1
                    && item.isAsterisk())
            {
        ...
        for (int i = 0; i < size; i++)
        {
            ...
            {
                processOrderByColumn(source, i, si, selectItemByIdentifier, newSelectItems, newSortItems, newComputedExpressions);
            }
        }

        return new Select(newSelectItems, source.getFrom(), source.getInto(), source.getTopExpression(), source.getWhere(), source.getGroupBy(), newSortItems, source.getForOutput(),
                newComputedExpressions);
    }

    private boolean processOrderByOrdinal(Select source, int index, SortItem si, int firstAsteriskOrdinal, List<SortItem> sortItems)
    {
        int ordinal = ((LiteralIntegerExpression) si.getExpression()).getValue();

        // Ordinal is 1-based
        if (ordinal < 1
                || ordinal > source.getSelectItems()
                        .size())
        {
        ...
        if (firstAsteriskOrdinal != -1
                && ordinal >= firstAsteriskOrdinal)
        {
            throw new ParseException("ORDER BY position is not supported for non expression select items", si.getToken());
        }

        ExpressionSelectItem selectItem = (ExpressionSelectItem) source.getSelectItems()
                .get(ordinal - 1);

        // Replace the ordinal sort item with a expression sort item
        sortItems.set(index, new SortItem(selectItem.getExpression(), si.getOrder(), si.getNullOrder(), si.getToken()));

        return selectItem.isComputed();
    }

    private void processOrderByColumn(Select source, int index, SortItem si, Map<String, Integer> computedItemByIdentifier, List<SelectItem> newSelectItems, List<SortItem> newSortItems,
            List<Expression> newComputedExpressions)
    {
        ...
        if (esi.isComputed())
        {
            int tupleOrdinal = source.getFrom() != null ? source.getFrom()
                    .getTableSource()
                    .getTableAlias()
                    .getTupleOrdinal()
                    : -1;
            QualifiedReferenceExpression newQre = new QualifiedReferenceExpression(QualifiedName.of(String.format("expr_%d_%d", tupleOrdinal, newComputedExpressions.size())), -1,
                    new ResolvePath[] { new ResolvePath(-1, tupleOrdinal, emptyList(), TableMeta.MAX_COLUMNS + newComputedExpressions.size()) }, null);

            newComputedExpressions.add(esi.getExpression());

            newSelectItems.set(itemIndex, new ExpressionSelectItem(newQre, esi.isEmptyIdentifier(), esi.getIdentifier(), null, esi.getToken()));

            newSortItems.set(index, new SortItem(newQre, si.getOrder(), si.getNullOrder(), si.getToken()));
        }
        // Non computed, simple change the sort item to the select items expression
        else
        {
            newSortItems.set(index, new SortItem(esi.getExpression(), si.getOrder(), si.getNullOrder(), si.getToken()));
        }
    }
}
