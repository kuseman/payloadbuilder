...
class BatchMergeJoin extends AOperator
{
    ...
    BatchMergeJoin(
            // CSON
            int nodeId, String logicalOperator, Operator outer, Operator inner, ValuesExtractor outerValuesExtractor, ValuesExtractor innerValuesExtractor, Predicate<ExecutionContext> predicate,
            TupleMerger rowMerger, boolean populating, boolean emitEmptyOuterRows, Index innerIndex, int batchSize)
    {
        super(nodeId);
        ...
        this.valuesSize = innerIndex.getColumns()
                .size();
        ...
        // CSON
        {
            ...
            // CSON
            {
                while (next == null)
                {
                    ...
                    if (innerRow == null)
                    {
                        innerRow = innerIt.hasNext() ? innerIt.next()
                                : null;
                        if (innerRow == null)
                        {
                            verifyOuterValuesIterator();
                            ...
                            else if (emitEmptyOuterRows
                                    && outerIndex < outerRows.size() - 1)
                            {
                    ...
                    int c = 0; // compare(context, outerRow, innerRow);
            ...
            private void populateKeyValues(ValuesExtractor valueExtractor, ExecutionContext context, Tuple tuple)
            {
                ...
                // CSON
                {
                    ...
                    private boolean setNext()
                    {
                        while (nextArray == null)
                        {
                            ...
                            if (prevArray != null
                                    && prevArray.equals(tuple.extractedValues))
                            {
                                continue;
                            }
    ...
    public String toString(int indent)
    {
        ...
        String description = String.format("BATCH MERGE JOIN (%s) (ID: %d, POPULATING: %s, OUTER: %s, BATCH SIZE: %s, PREDICATE: %s)", logicalOperator, nodeId, populating, emitEmptyOuterRows,
                batchSize, predicate);
