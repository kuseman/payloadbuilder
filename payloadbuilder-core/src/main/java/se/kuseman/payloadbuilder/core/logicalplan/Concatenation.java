...
public class Concatenation implements ILogicalPlan
{
    ...
    public Concatenation(List<String> columnNames, List<ILogicalPlan> inputs, Location location)
    {
        ...
        if (inputs.size() <= 1)
        {
            throw new IllegalArgumentException("Number in inputs must be greater than 1");
        }
    ...
    private Schema verifySchema()
    {
        ...
        {
            throw new ParseException("Colum alias count must equal inputs column size", location);
        }
