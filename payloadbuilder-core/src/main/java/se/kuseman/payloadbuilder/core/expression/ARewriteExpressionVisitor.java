...
public abstract class ARewriteExpressionVisitor<C> extends AExpressionVisitor<IExpression, C>
{
    ...
    {
        return new UnresolvedFunctionCallExpression(expression.getCatalogAlias(), expression.getName(), expression.getAggregateMode(), visit(expression.getChildren(), context),
    ...
    {
        DereferenceExpression de = (DereferenceExpression) expression;
        return new DereferenceExpression(visit(expression.getExpression(), context), expression.getRight(), de.getOrdinal(), expression.getType(), de.getColumnReference());
    }
