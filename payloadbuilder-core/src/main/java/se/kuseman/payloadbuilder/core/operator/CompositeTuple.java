...
class CompositeTuple implements Tuple
{
    ...
    CompositeTuple add(Tuple tuple)
    {
        ...
        {
            /*
             * select * from tableA a 0 inner join 1 ( select * from tableB b 2 inner join tableC c 3 on ... ) b on ... inner join 4 ( select * from tableD d 5 inner join tableE e 6 on ... ) c on ...
             *
             *
             * this tuples row a (tuple 1) row b (tuple 2 starts here) row c null (tuple 3 starts here) null
             *
             * tuple 1 (ordinal 0) (row) row a (ordinal 0)
             *
             * tuple 2 (ordinal 1) (sub query) row b (ordinal 2) row c (ordinal 3)
             *
             * tuple 3 (ordinal 4) (sub query) row d (ordinal 5) row e (ordinal 6)
             *
             * If the ordinal in this is occupied we need to make a copy and overwrite tuples starting at the ordinal index
             *
             */
             ...
             *
             * ie. index = 3 (tuple 1 and 2 added) input tuple is tuple 1 which has length of 2 (we are about to add a tuple that should trigger a copy)
             *
             * checkIndex = 3 - 2 => 1 if tuple at index = 1 has the same ordinal as the first tuple we are about to add, make a copy
             */

            int checkIndex = index - ci.tuples.length;
            Tuple tupleToCheck = checkIndex >= 0 ? tuples[checkIndex]
                    : null;

            // We already have tuples with target ordinal at position so make a copy and alter the insert index
            if (tupleToCheck != null
                    && tupleToCheck.getTupleOrdinal() == ci.tuples[0].getTupleOrdinal())
            {
        ...
        else
        {
            ...
            if (index > 0
                    && tuples[index - 1].getTupleOrdinal() == tuple.getTupleOrdinal())
            {
    ...
    public Tuple getTuple(int tupleOrdinal)
    {
         ...
         * Ex. composite layout
         *
         * tableA ordinal 0 subQeury ordinal 1 tableB ordinal 2 tableC ordinal 3 tableD ordinal 4
         *
         * Wanted ordinal is 3 that isn't located in this tuple but in one of it's children
         */

        /*
         * select x.id <- ordinal 2 , y.id <- ordinal 4 from tableA inner join () x with (pop) on ... inner join () y with (pop) on ...
         *
         * CompositeTuple Row (a) or = 0 Collection (x) or = 1 Collection (y) or = 3
         */
