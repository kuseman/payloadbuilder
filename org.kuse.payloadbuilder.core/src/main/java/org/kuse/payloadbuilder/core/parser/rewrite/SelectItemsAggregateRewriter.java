...
class SelectItemsAggregateRewriter implements SelectRewriter<Select>
{
    ...
    public Select rewrite(Select source, Context context)
    {
        ...
        /*
         * Aggregate select items from sub queries from either the table source
         * or joins
         *
         * select *                    (ordinals = 0, 1)
         * from table a
    ...
    private boolean shouldAggregate(Select select)
    {
        ...
        if (select.getFrom().getTableSource() instanceof SubQueryTableSource)
        {
            return true;
        }

        for (AJoin join : select.getFrom().getJoins())
        {
    ...
    private void aggregateSelectItems(
            Set<Integer> asteriskAliasOrdinals,
            List<SelectItem> selectItems,
            List<Integer> ordinals,
            TableSource ts)
    {
        if (!asteriskAliasOrdinals.contains(ts.getTableAlias().getTupleOrdinal()))
        {
            return;
        }

        /*
         * Sub query, then pick all select items from sub query
         * If an asterisk item is encountered, add it's ordinals to result list
         */
        if (ts instanceof SubQueryTableSource)
        {
            SubQueryTableSource sts = (SubQueryTableSource) ts;
            // Add all sub query items
            selectItems.addAll(sts.getSelect().getSelectItems());
        }
        /* Ordinary table source, just add it's ordinal to asterisk list */
        else
        {
            ordinals.add(ts.getTableAlias().getTupleOrdinal());
        }
    }
